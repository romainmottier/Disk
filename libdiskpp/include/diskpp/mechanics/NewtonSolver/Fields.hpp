...
class TimeField {
  ...
  public:
     ...
     *
     */
    void setTime( const scalar_type &time ) { m_time = time; }

    void setField( const FieldName &name, const std::vector< vector_type > &field ) {
        m_fields[name] = field;
    }

    auto getField( const FieldName &name ) const {
        // std::cout << getFieldName(name) << std::endl;
        return m_fields.at( name );
    }

    void clearField( const FieldName &name ) {
    ...
    void createZeroField( const FieldName &name, const Mesh &mesh,
    ...
    template < typename Mesh >
    void createField( const FieldName &name, const Mesh &mesh,
                      const MeshDegreeInfo< Mesh > &degree_infos,
...
class MultiTimeField {
  ...
  public:
    ...
    void setCurrentTime( const scalar_type &time ) { m_fields.at( 0 ).setTime( time ); }
    ...
    auto getField( const int &relative_index, const FieldName &name ) const {
    ...
    auto getCurrentField( const FieldName &name ) const {
        return m_fields.at( 0 ).getField( name );
    }

    void setField( const int &relative_index, const FieldName &name,
    ...
    template < typename Mesh >
    void createField( const int &relative_index, const FieldName &name, const Mesh &mesh,
                      const MeshDegreeInfo< Mesh > &degree_infos,
                      const vector_rhs_function< Mesh > func ) {
