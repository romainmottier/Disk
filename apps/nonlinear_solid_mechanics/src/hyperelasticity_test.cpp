...
error_type run_hyperelasticity_solver( const Mesh< T, 2, Storage > &msh,
    ...
    auto load = [material_data, alpha]( const disk::point< T, 2 > &p,
    ...
    auto solution = [material_data, alpha]( const disk::point< T, 2 > &p ) -> result_type {
        ...
        T uy = ( 1.0 / material_data.getLambda() - alpha / ( 1.0 + alpha ) ) * p.y() +
               2 * alpha * ( cos( 2 * M_PI * p.x() ) - 1.0 );
...
template < template < typename, size_t, typename > class Mesh, typename T, typename Storage >
error_type run_hyperelasticity_solver( const Mesh< T, 3, Storage > &msh,
                                       const disk::mechanics::NonLinearParameters< T > &rp,
                                       const disk::mechanics::MaterialData< T > &material_data ) {
    ...
    auto load = [material_data, alpha, gamma]( const disk::point< T, 3 > &p,
                                               const T &time ) -> result_type {
        const T lambda = material_data.getLambda();
        const T mu = material_data.getMu();
    ...
    auto solution = [material_data, alpha, gamma]( const disk::point< T, 3 > &p ) -> result_type {
        ...
        T uy = -( 1.0 / material_data.getLambda() +
                  ( alpha + gamma + alpha * gamma ) / ( 1.0 + alpha + gamma + alpha * gamma ) ) *
               p.y();
