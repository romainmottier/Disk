...
void renumber_boundaries_2d( Mesh &msh ) {
...
template < typename Mesh >
void renumber_boundaries_3d( Mesh &msh ) {
    using T = typename Mesh::coordinate_type;
    auto storage = msh.backend_storage();
...
error_type run_tresca_solver( Mesh< T, 2, Storage > &msh,
    ...
    auto load = [material_data]( const disk::point< T, 2 > &p, const T &time ) -> result_type {
        ...
        T fx =
            -mu * ( lambda * y + x * coeff ) + y * ( lambda + mu * ( lambda + 2 ) ) * ( x * y + 2 );
        T fy = -lambda * x * ( mu - 1 ) * ( x * y + 2 ) +
               mu * ( 2 * x * ( 0.5 * lambda + 1 ) - y * coeff );
    ...
    auto sol = [solution]( const disk::point< T, 2 > &p ) -> auto { return solution( p, 1.0 ); };
...
template < template < typename, size_t, typename > class Mesh, typename T, typename Storage >
error_type run_tresca_solver( const Mesh< T, 3, Storage > &msh,
                              const disk::mechanics::NonLinearParameters< T > &rp,
                              const disk::mechanics::MaterialData< T > &material_data ) {
    ...
    auto load = [material_data]( const disk::point< T, 3 > &p, const T &time ) -> result_type {
        ...
        T fx =
            -mu * ( lambda * z + x * coeff ) + z * ( lambda + mu * ( lambda + 2 ) ) * ( x * z + 2 );
        T fz = -lambda * x * ( mu - 1 ) * ( x * z + 2 ) +
               mu * ( 2 * x * ( 0.5 * lambda + 1 ) - z * coeff );

        return -time * result_type { fx, 0.0, fz } * exz /
               ( 3.0 * ( 1.0 + material_data.getLambda() ) );
    };
    ...
    auto solution = [material_data]( const disk::point< T, 3 > &p, const T &time ) -> result_type {
        ...
        return time * result_type { x * exz * ( 1.0 + coeff ), 0.0, z * exz * ( -1.0 + coeff ) } /
               6.0;
    };

    auto sol = [solution]( const disk::point< T, 3 > &p ) -> auto { return solution( p, 1.0 ); };
...
void printResults( const std::vector< error_type > &error ) {
    if ( error.size() > 0 ) {
        ...
        std::cout
            ...
            << std::endl;
        std::cout
            << "| Size mesh  | Displacement | Convergence | Displacem  | Convergence |    Total   |"
            << std::endl;
        std::cout
            << "|    h       |   L2 error   |     rate    |  H1 error  |     rate    | faces DOF  |"
            << std::endl;
        std::cout
            << "-----------------------------------------------------------------------------------"
            << std::endl;
...
void printResults2( const std::vector< error_type > &error ) {
    if ( error.size() > 0 ) {
        std::ios::fmtflags f( std::cout.flags() );
        std::cout.precision( 4 );
        std::cout.setf( std::iostream::scientific, std::iostream::floatfield );
        ...
        std::cout
            ...
            << std::endl;
        std::cout
            << "| Lambda coef| Displacement | Difference  | Displacem  | Difference  |    Total   |"
            << std::endl;
        std::cout << "|    nu       |   L2 error   |             |  H1 error  |             | "
                     "faces DOF  |"
                  << std::endl;
        std::cout
            << "-----------------------------------------------------------------------------------"
...
void test_quads_diskpp( const disk::mechanics::NonLinearParameters< T > &rp,
    ...
    paths.push_back( mesh_base + "/2D_quads/diskpp/testmesh-2-2.quad" );
...
template < typename T >
void test_triangles_fvca5_robust( const disk::mechanics::NonLinearParameters< T > &rp,
...
template < typename T >
void test_quads_diskpp_robust( const disk::mechanics::NonLinearParameters< T > &rp,
                               disk::mechanics::MaterialData< T > &material_data ) {
...
template < typename T >
void test_hexagons_robust( const disk::mechanics::NonLinearParameters< T > &rp,
                           disk::mechanics::MaterialData< T > &material_data ) {
...
template < typename T >
void test_kershaws_robust( const disk::mechanics::NonLinearParameters< T > &rp,
                           disk::mechanics::MaterialData< T > &material_data ) {
...
template < typename T >
void test_tetrahedra_fvca6_robust( const disk::mechanics::NonLinearParameters< T > &rp,
                                   disk::mechanics::MaterialData< T > &material_data ) {
...
template < typename T >
void test_hexahedra_fvca6_robust( const disk::mechanics::NonLinearParameters< T > &rp,
                                  disk::mechanics::MaterialData< T > &material_data ) {
...
template < typename T >
void test_polyhedra_fvca6_robust( const disk::mechanics::NonLinearParameters< T > &rp,
                                  disk::mechanics::MaterialData< T > &material_data ) {
...
int main( int argc, char **argv ) {
    ...
    std::cout << "Material coefficients:  E = " << material_data.getE()
              << " , nu = " << material_data.getNu() << std::endl;

    // Solver parameters
    disk::mechanics::NonLinearParameters< RealType > rp;
    rp.m_precomputation = true;
    rp.m_stab_type = disk::mechanics::StabilizationType::HHO;
    rp.m_epsilon = 2.0E-6;
