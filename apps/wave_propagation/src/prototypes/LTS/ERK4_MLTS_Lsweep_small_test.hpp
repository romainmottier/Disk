...
void ERK4_MLTS_Lsweep_small_test(int argc, char **argv);
...
void ERK4_MLTS_Lsweep_small_test(int argc, char **argv)
{
    ...
    for (auto & thresholds : threshold_sets) {
        ...
        for (int i = 0; i < L; ++i) blocks[i] = erk_an.build_LTS_subblock(own_c[i], own_f[i], 3);

        // Band0's "uncovered" check must be tested against the UNION of
        // EVERY descendant band's halo (blocks[1..L-1]), not just blocks[1]
        // alone: in the revised design every level >=1 leaks into its own
        // halo via erk_weight_LTS_fine_v2's full-active-set scatter, so a
        // band0 cell can be genuinely "covered" (have real B(Pfine*y)!=0
        // coupling) by band2's, band3's, ... leak while sitting entirely
        // outside band1's (now much narrower, once band1 itself gets split
        // finer) halo. Checking against blocks[1] alone silently applies
        // the closed-form integral (exact ONLY for cells with zero fine
        // coupling) to interface cells that DO have such coupling -- this
        // is exactly what produced the band0-own-dofs error jumping from
        // ~2e-5 (L=3, where blocks[1] alone still spans everything finer)
        // to ~8e-3 (L=4/5, where blocks[1] narrowed and stopped reaching
        // some genuinely-interface cells).
        std::vector<int> descendant_active_c_all, descendant_active_f_all;
        for (int b = 1; b < L; ++b) {
            descendant_active_c_all.insert(descendant_active_c_all.end(), blocks[b].active_c.begin(), blocks[b].active_c.end());
            descendant_active_f_all.insert(descendant_active_f_all.end(), blocks[b].active_f.begin(), blocks[b].active_f.end());
        }
        ...
        recurse = [&](int i, RealType t_start, RealType len, Matrix<RealType,Dynamic,1>& xn, const std::vector<Matrix<RealType,Dynamic,1>>& ancestor_w) {
            if (i == 0) {
                // Root: same "recompute fresh every short sub-step" treatment as
                // intermediate levels (not one fit extrapolated across the whole
                // macro-step) -- Grote-Diaz's base-case LTS2, chained p times via
                // its own leap-frog recurrence, achieves p-hop reach for its OWN
                // (coarse) rows too; a single frozen fit for the WHOLE macro-step
                // does not replicate that chained-reach property.
                int p_0 = p_level[0];
                ...
                for (int m = 0; m < p_0; ++m) {
                    ...
                    Matrix<RealType, Dynamic, 1> Fn = eval_F(t_start + tm), Fn12 = eval_F(t_start + tm + 0.5*dtau_0), Fn1 = eval_F(t_start + tm + dtau_0);
                    erk_an.erk_weight_LTS_coarse_v2(xn, blocks[0], own_w, Fn, Fn12, Fn1, dtau_0);
                    if (L > 1) erk_an.erk_weight_LTS_coarse_advance_uncovered(xn, blocks[0], own_w, blocks[L-1].active_c, blocks[L-1].active_f, dtau_0);
                    recurse(1, t_start + tm, dtau_0, xn, own_w);
                }
                return;
            }
            if (i == L - 1) {
                // Terminal level only: genuine RK4 step, forced by the inherited
                // (already-combined) ancestor forcing. Matches Grote-Diaz: only
                // the finest level gets real time integration.
            ...
            // Intermediate level (1 <= i <= L-2): NO dedicated fine_v2 step here
            // (Grote-Diaz-faithful -- only the terminal level integrates; this
            // level's own dofs evolve purely via the leak from deeper levels'
            // genuine steps, plus its own closed-form "uncovered" update).
            // Its "coarse role" fit is recomputed FRESH, from the CURRENT xn,
            // at EVERY short sub-step (not once then Taylor-extrapolated across
            // the whole `len`) -- this is the structural fix: Grote-Diaz recompute
            // each level's contribution to its descendant from the actual,
            // up-to-date state at every sub-step, never from an extrapolated
            // polynomial spanning many octaves of descendant refinement.
            {
                int p_i = p_level[i];
                RealType dtau_i = len / p_i;
                for (int m = 0; m < p_i; ++m) {
                    RealType tm = m * dtau_i;
                    auto shifted_ancestor = taylor_shift(ancestor_w, tm);
                    ...
                    erk_an.erk_weight_LTS_coarse_v2(xn, blocks[i], own_w_i, Fn, Fn12, Fn1, dtau_i, &shifted_ancestor);
                    // Coverage must be checked against the TERMINAL level's halo,
                    // not the immediate next band: since intermediate levels no
                    // longer have their own fine_v2 leak, blocks[i+1] (if i+1 is
                    // itself intermediate) provides NO real leak -- checking
                    // against it would wrongly exclude cells that then never get
                    // touched by anything (this was the bug that made band0/band1
                    // blow up in the first Grote-Diaz-faithful attempt).
                    erk_an.erk_weight_LTS_coarse_advance_uncovered(xn, blocks[i], own_w_i, blocks[L-1].active_c, blocks[L-1].active_f, dtau_i);
                    auto shifted = own_w_i;
