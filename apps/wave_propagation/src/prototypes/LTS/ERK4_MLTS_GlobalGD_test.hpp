...
void ERK4_MLTS_GlobalGD_test(int argc, char **argv);
...
void ERK4_MLTS_GlobalGD_test(int argc, char **argv)
{
    ...
    for (auto & thresholds : threshold_sets) {
        ...
        // band b's own dofs, 0 elsewhere -- these are DISJOINT (bands
        // partition the mesh), unlike Grote-Diaz's NESTED P_l (level l or
        // finer); band b's own indicator is the direct analog of their
        // (P_l - P_{l+1}).
        ...
        for (int b = 0; b < L; ++b) {
            ...
            P_band[b] = SparseMatrix<RealType>(n_dof, n_dof);
        ...
        // Grote-Diaz-faithful, GLOBAL-matrix recursion (Algorithm 4, RK4
        // variant): every level i < L-1 recomputes its OWN contribution
        // fresh, from the CURRENT global state, at every short sub-step,
        // and ACCUMULATES it (sums, exactly like their v := w + Sum w_k)
        // with what it inherited from its ancestors before Taylor-shifting
        // and recursing. No restricted sub-blocks, no separate "advance
        // uncovered" step: every level's own dofs are updated purely
        // through the leak of the GLOBAL Kcc/Kcf/Kfc operators, chained
        // across the recursion's many nested visits -- exactly as in the
        // paper, where only the base-case LTS2/LTS4 call ever directly
        ...
        recurse = [&](int i, RealType t_start, RealType len, Matrix<RealType,Dynamic,1>& xn, const std::vector<Matrix<RealType,Dynamic,1>>& w_accum) {
            if (i == L - 1) {
                ...
                erk_an.erk_weight_LTS_fine(xn, P_band[L-1], w_accum, Fm, Fmh, Fm1, 0.0, len);
                return;
            }
            int p_i = p_level[i];
            ...
            for (int m = 0; m < p_i; ++m) {
                ...
                erk_an.erk_weight_LTS_coarse_global(xn, P_band[i], own_w_i, Fn, Fn12, Fn1, dtau_i);
