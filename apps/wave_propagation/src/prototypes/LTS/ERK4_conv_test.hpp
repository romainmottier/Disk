...
void ERK4_LTS_conv_test(int argc, char **argv){
    ...
    for(size_t it = 1; it <= nt; it++) {
        ...
        // Butcher tableau offsets for RK4: 
        ...
        //         double t_abs = (m + c[s]) * dtau;
                
        //         // Manufactured solution
        //         auto v_fun      = functions.Evaluate_v(t_abs);
        //         auto f_fun      = functions.Evaluate_f(t_abs);
        //         auto s_v_fun    = functions.Evaluate_s_v(t_abs);
        //         auto s_f_fun    = functions.Evaluate_s_f(t_abs);   
        ...
        //         double t3 = t_abs * t2;
        //         k[s] += w[0] + t_abs*w[1] + t2*w[2]/2.0 + t3*w[3]/6.0;
        //     }
        ...
        //     // Terme source fin aux 3 temps du sous-pas (temps ABSOLU)
        //     auto fine_F = [&](double tau) {
        //         auto f_fun   = functions.Evaluate_f(tn + tau);
        //         auto s_f_fun = functions.Evaluate_s_f(tn + tau);
        //         assembler.assemble_rhs(msh, f_fun, s_f_fun, false);
        //         Matrix<RealType, Dynamic, 1> PF(n_dof); PF.setZero();
        //         PF.block(0, 0, m_n_c_dof, 1) = erk_an.invMc() 
        //         * (assembler.Pfine * assembler.RHS).block(0, 0, m_n_c_dof, 1);
        //         return PF;
        //     };
            
        //     auto PFm  = fine_F(tm);
        //     auto PFmh = fine_F(tmh);
        //     auto PFm1 = fine_F(tm1);
        ...
        //     // k1: ỹ_m = Pfine * x_dof_n
        //     yn[0] = assembler.Pfine * x_dof_n;
        //     erk_an.erk_weight_without_F(yn[0], k[0]);
        //     k[0] += Taylor_w(tm) + PFm;
            
        //     // k2: ỹ + Δτ/2 * k1
        //     yn[1] = assembler.Pfine * (x_dof_n + dtau/2.0 * k[0]);
        //     erk_an.erk_weight_without_F(yn[1], k[1]);
        //     k[1] += Taylor_w(tmh) + PFmh;
            
        //     // k3: ỹ + Δτ/2 * k2
        //     yn[2] = assembler.Pfine * (x_dof_n + dtau/2.0 * k[1]);
        //     erk_an.erk_weight_without_F(yn[2], k[2]);
        //     k[2] += Taylor_w(tmh) + PFmh;
            
        //     // k4: ỹ + Δτ * k3
        //     yn[3] = assembler.Pfine * (x_dof_n + dtau * k[2]);
        //     erk_an.erk_weight_without_F(yn[3], k[3]);
        //     k[3] += Taylor_w(tm1) + PFm1;
            
        //     // FINAL UPDATE
        //     x_dof_n += dtau * (k[0] + 2.0*k[1] + 2.0*k[2] + k[3]) / 6.0;
        // }
        // Evaluation du terme source coarse aux temps tn, tn+1/2, tn+1
auto eval_F_coarse = [&](RealType t_abs) -> Matrix<RealType, Dynamic, 1> {
    auto v_fun   = functions.Evaluate_v(t_abs);
    auto f_fun   = functions.Evaluate_f(t_abs);
    auto s_v_fun = functions.Evaluate_s_v(t_abs);
    auto s_f_fun = functions.Evaluate_s_f(t_abs);
    ...
    // Evaluation du terme source fin aux temps du sous-pas
    auto eval_F_fine = [&](double tau_local) -> Matrix<RealType, Dynamic, 1> {
        double t_abs = tn + tau_local;
        auto v_fun   = functions.Evaluate_v(t_abs);
        auto f_fun   = functions.Evaluate_f(t_abs);
        auto s_v_fun = functions.Evaluate_s_v(t_abs);
        auto s_f_fun = functions.Evaluate_s_f(t_abs);
    ...
    Matrix<RealType, Dynamic, 1> Fm  = eval_F_fine(tm);
    Matrix<RealType, Dynamic, 1> Fmh = eval_F_fine(tmh);
    Matrix<RealType, Dynamic, 1> Fm1 = eval_F_fine(tm1);
    ...
    // ỹ_m = Pfine * x_dof_n
    Matrix<RealType, Dynamic, 1> y_tilde = assembler.Pfine * x_dof_n;

    // k1
    erk_an.erk_weight_LTS_fine(y_tilde, assembler.Pfine, Fm, k[0]);
    k[0] += Taylor_w(tm);

    // k2
    Matrix<RealType, Dynamic, 1> y2 = y_tilde + dtau/2.0 * k[0];
    erk_an.erk_weight_LTS_fine(y2, assembler.Pfine, Fmh, k[1]);
    k[1] += Taylor_w(tmh);

    // k3
    Matrix<RealType, Dynamic, 1> y3 = y_tilde + dtau/2.0 * k[1];
    erk_an.erk_weight_LTS_fine(y3, assembler.Pfine, Fmh, k[2]);
    k[2] += Taylor_w(tmh);

    // k4
    Matrix<RealType, Dynamic, 1> y4 = y_tilde + dtau * k[2];
    erk_an.erk_weight_LTS_fine(y4, assembler.Pfine, Fm1, k[3]);
    k[3] += Taylor_w(tm1);
