...
void ERK4_MLTS_L5small_v2design_validation_test(int argc, char **argv);

void ERK4_MLTS_L5small_v2design_validation_test(int argc, char **argv)
{
    std::cout << std::endl << bold << red << "   MULTI-LEVEL DRIVER VALIDATION (L=5, N=1, small p_global)" << std::endl << std::endl;
    ...
    // approximation error).
    // p_global=32=2^5 exactly, split into L=5 bands (~1 octave each,
    // matching the earlier 1-octave-per-level attempt that diverged at
    // N=2 -- testing it here, small and controlled, isolates whether the
    // problem scales with L (levels) or with p_global (mesh depth).
    // p0=p1=p2=2, p3=4 (product=32=p_global exactly, same
    // exact-quantization-matching trick as the L=3 test).
    std::vector<RealType> thresholds = { h_max / 2.0, h_max / 4.0, h_max / 8.0, 2.0 * h_min };
    const int L = 5;
    ...
    recurse = [&](int i, RealType t_start, RealType len, Matrix<RealType,Dynamic,1>& xn, const std::vector<Matrix<RealType,Dynamic,1>>& ancestor_w) {
        ...
        if (has_own_w) {
            ...
            // Band i's own "coarse role" fit must reflect its TRUE dynamics,
            // which includes the ancestor chain's ongoing forcing
            // (ancestor_w) -- NOT just band i's own local operator +
            // external source F. Pass ancestor_w through to the
            // (unit-consistent) library-side correction rather than folding
            // it into Fn (an earlier attempt did that and blew up: Fn is a
            // RAW force, ancestor_w is already Mc_inv-scaled -- adding them
            // and then applying Mc_inv again is a units bug).
            Matrix<RealType, Dynamic, 1> Fn = eval_F(t_start), Fn12 = eval_F(t_start + 0.5*len), Fn1 = eval_F(t_start + len);
            erk_an.erk_weight_LTS_coarse_v2(xn, blocks[i], own_w_i, Fn, Fn12, Fn1, len, &ancestor_w);
        }
    ...
    for (int b = 1; b < L; ++b) {
        for (int g : own_c[b]) tf_trip.emplace_back(g, g, 1.0);
        for (int g : own_f[b]) tf_trip.emplace_back((int)erk_an.n_c_dof()+g, (int)erk_an.n_c_dof()+g, 1.0);
    }
