...
void ERK4_MLTS_DiagCompare_test(int argc, char **argv);
...
void ERK4_MLTS_DiagCompare_test(int argc, char **argv)
{
    ...
    for (auto & thresholds : threshold_sets) {
        ...
        if (L == 3) {
            SparseMatrix<RealType> P0(n_dof, n_dof);
            std::vector<Eigen::Triplet<RealType>> trip;
            for (int g : own_c[0]) trip.emplace_back(g, g, 1.0);
            for (int g : own_f[0]) trip.emplace_back((int)erk_an.n_c_dof()+g, (int)erk_an.n_c_dof()+g, 1.0);
            P0.setFromTriplets(trip.begin(), trip.end());

            RealType dtau0 = dt_macro / p_level[0];
            Matrix<RealType,Dynamic,1> Fn = eval_F(ti), Fn12 = eval_F(ti+0.5*dtau0), Fn1 = eval_F(ti+dtau0);

            std::vector<Matrix<RealType,Dynamic,1>> w_glob(4), w_restr(4);
            for (int j=0;j<4;++j) { w_glob[j]=Matrix<RealType,Dynamic,1>::Zero(n_dof); w_restr[j]=Matrix<RealType,Dynamic,1>::Zero(n_dof); }
            Matrix<RealType,Dynamic,1> xn0 = x_dof0;
            erk_an.erk_weight_LTS_coarse_global(xn0, P0, w_glob, Fn, Fn12, Fn1, dtau0);
            erk_an.erk_weight_LTS_coarse_v2(xn0, blocks[0], w_restr, Fn, Fn12, Fn1, dtau0);
            for (int j=0;j<4;++j) {
                RealType d = (w_glob[j]-w_restr[j]).cwiseAbs().maxCoeff();
                RealType mg = w_glob[j].cwiseAbs().maxCoeff();
                RealType mr = w_restr[j].cwiseAbs().maxCoeff();
                std::cout << bold << yellow << "      [DIAG] j=" << j << " diff=" << d << " max|global|=" << mg << " max|restr|=" << mr << reset << std::endl;
            }
        }
        ...
        recurse = [&](int i, RealType t_start, RealType len, Matrix<RealType,Dynamic,1>& xn, const std::vector<Matrix<RealType,Dynamic,1>>& w_accum) {
            if (i == L - 1) {
                // Terminal level only: genuine RK4 step, forced by the fully
                // accumulated ancestor forcing (sum of every level 0..L-2's
                // own contribution). Matches Grote-Diaz: only the finest
                // level gets real time integration.
            ...
            for (int m = 0; m < p_i; ++m) {
                ...
                erk_an.erk_weight_LTS_coarse_v2(xn, blocks[i], own_w_i, Fn, Fn12, Fn1, dtau_i);
