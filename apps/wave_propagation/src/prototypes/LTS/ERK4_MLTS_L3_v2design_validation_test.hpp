...
void ERK4_MLTS_L3_v2design_validation_test(int argc, char **argv);

void ERK4_MLTS_L3_v2design_validation_test(int argc, char **argv)
{
    std::cout << std::endl << bold << red << "   MULTI-LEVEL DRIVER VALIDATION -- REVISED DESIGN (every level>=1 gets real RK4) (L=3, N=1)" << std::endl << std::endl;
    ...
    // REVISED design (per user clarification): level 0 (coarsest) is
    // NEVER genuinely time-stepped -- only its own cubic-Taylor fit,
    // closed-form-integrated for whatever isn't covered by level 1's
    // halo. EVERY level i>=1 (not just the terminal one) gets a REAL RK4
    // step (erk_weight_LTS_fine_v2), using the frozen w handed down from
    // level i-1 as the additive forcing -- exactly one such step per
    // recursion node, of size `len` (that node's own interval, which by
    // construction of p_level already equals level i's own characteristic
    // dt_level[i]). Levels i<L-1 ALSO compute their own w_i (BEFORE their
    // own fine_v2 update mutates the state, matching the "frozen at
    // interval start" convention) to hand down to level i+1. Since every
    // level i>=1 now genuinely scatters over its own+halo, level 0's
    // coverage check is against blocks[1] specifically (the immediate
    // next level) -- not blocks[L-1] as in the old (single-terminal-scatter)
    // design; that fix was a workaround for a structure this design no
    // longer has.
    ...
    recurse = [&](int i, RealType t_start, RealType len, Matrix<RealType,Dynamic,1>& xn, const std::vector<Matrix<RealType,Dynamic,1>>& ancestor_w) {
        ...
        if (i == 0) {
            ...
            erk_an.erk_weight_LTS_coarse_advance_uncovered(xn, blocks[0], own_w, blocks[1].active_c, blocks[1].active_f, len);
        ...
        // i >= 1: genuinely advance THIS level's own dofs, one RK4 step of
        // size len, using the ancestor's (already locally-shifted) w.
        ...
        if (!has_own_w) return; // i == L-1, terminal: nothing to recurse into
