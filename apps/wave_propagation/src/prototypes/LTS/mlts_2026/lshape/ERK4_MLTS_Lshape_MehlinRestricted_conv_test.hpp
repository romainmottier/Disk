...
#include <functional>
//
// Same recursion/orchestration as ERK4_MLTS_Lshape_RestrictedExact_conv_test.hpp
// (erk_weight_LTS_coarse_v2 + erk_weight_LTS_coarse_advance_uncovered_full
// for i<L-1, erk_weight_LTS_fine_v2 for the terminal level) -- ONLY the
// per-band submatrix construction changes.
//
// Side-by-side comparison across four drivers this session pinned down
// the real cause of RestrictedExact's ~5.9x residual: it is NOT the
// B-chain's masking convention (masked P_ell-chain vs unmasked v2-chain
// give essentially IDENTICAL error once overlap is added to both -- see
// ERK4_MLTS_Lshape_RestrictedLeveled_conv_test.hpp), it is
// build_LTS_subblock's FIXED, GUESSED halo width (8 rings; 20 rings
// tested WORSE, not better) not always being a large-enough
// neighbourhood for the B-chain (up to B^3) to reproduce the true
// global operator's action at a band's own dofs. GlobalExact and the
// literal Mehlin reproduction (both using the FULL, untruncated global
// matrices) match the trusted reference to <0.5%, confirming the
// algorithm itself is right -- only the submatrix's REACH was wrong.
//
// Fix (in the spirit of Almquist-Mehlin's own P_ell/eq. 20-22
// construction, but applied to WHERE the submatrix domain comes from
// rather than to per-step masking): build each band's submatrix from
// build_LTS_subblock_Pell(own_i, Pell_i, ...), where Pell_i = own_i
// UNION every FINER band's own dofs (nested inside band i spatially on
// this graded corner mesh) UNION a few overlap rings into the coarser
// side (Almquist-Mehlin Section 3.2/Table 2: 3 rings for RK4) --
// instead of guessing a ring count. This reach still shrinks with band
// index (fewer bands remain "finer" as i grows), so it stays a genuine
// per-call cost reduction relative to the always-O(n_dof) GlobalExact/
// MehlinExact drivers.
//
// Usage example: ../../../wave_propagation -k3 -s0 -r0 -c0 -f0 -e0

void ERK4_MLTS_Lshape_MehlinRestricted_conv_test(int argc, char **argv);
...
void ERK4_MLTS_Lshape_MehlinRestricted_conv_test(int argc, char **argv)
{
    ...
    conv_fname << out_dir << "/lshape_mlts_convergence_mehlinrestricted_k_" << mesh_k << ".txt";
    ...
    for (int N : level_N) {
        ...
        // Pell_c[b]/Pell_f[b] = P_b in the paper: band b's own dofs UNION
        // every FINER band's own dofs (nested inside band b spatially on
        // this graded corner mesh) -- built cumulatively from the finest
        // level down, same construction as ERK4_MLTS_Lshape_MehlinExact_conv_test.hpp.
        ...
        // Overlap (Almquist-Mehlin Section 3.2, Table 2: 3 rings for
        // RK4): widen every band's domain (INCLUDING the terminal band,
        // whose Pell_{L-1} is otherwise just its own dofs) by a few BFS
        // hops through the Kfc sparsity graph, reaching a few extra dofs
        // on the COARSER side beyond each band's strict boundary -- this
        // is what gives the terminal band ("fine" role) the interface
        // halo it needs for its own leak-into-neighbour write (the old
        // Bug A/B fix), replacing build_LTS_subblock's guessed
        // halo_rings=8 with the same principled mechanism used
        // everywhere else.
        ...
        // RESTRICTED per-band sub-blocks, now built from the EXACT
        // Pell_c/Pell_f domain (own + every finer band + overlap)
        // instead of build_LTS_subblock's fixed-width BFS-ring halo --
        // see build_LTS_subblock_Pell's own comment in
        // erk_coupling_hho_scheme.hpp for why this was the actual
        // missing piece. Reach shrinks with band index (own_c[b] union
        // shrinks as fewer bands remain "finer"), so this stays O(band
        // size), not O(n_dof), for i>=1 -- the actual performance
        // optimization on top of the (now hopefully) exact algorithm.
        std::vector<erk_coupling_hho_scheme<RealType>::LTS_subblock_set> blocks(L);
        for (int b = 0; b < L; ++b) {
            blocks[b] = erk_an.build_LTS_subblock_Pell(own_c[b], own_f[b], Pell_c[b], Pell_f[b]);
            std::cout << bold << cyan << "      band" << b << ": own_c=" << own_c[b].size()
                      << " own_f=" << own_f[b].size() << "  active_c=" << blocks[b].active_c.size()
                      << " active_f=" << blocks[b].active_f.size() << reset << std::endl;
        }
