...
// using erk_weight_LTS_coarse_mehlin instead of erk_weight_LTS_coarse_global:
// masks the B-chain to P_ell = "level ell AND every finer level" at
// EVERY application of B (not just once at the end, the way the
// simplified/reformulated erk_weight_LTS_coarse_global does). See that
// function's own comment in erk_coupling_hho_scheme.hpp for the full
// derivation and why the ancestor cross-term sum of eq. (22) is exactly
// reproduced by the driver's existing incremental Taylor-shift
// `combined` accumulation (no simplification lost there).
//
// PURPOSE: pure fidelity validation, not performance. If our
// reformulated GlobalExact driver is a valid algebraic simplification
// of the paper's literal construction (as the machine-precision
// agreement already suggests), this driver should reproduce the EXACT
// SAME numbers. If it doesn't, the simplification is wrong somewhere.
// Uses GLOBAL matrices throughout -- P_ell masking to "all finer
// levels" cannot be represented by any FIXED-width local halo, so this
// is deliberately as expensive as (or more expensive than) GlobalExact.
//
// Usage example: ../../../wave_propagation -k3 -s0 -r0 -c0 -f0 -e0

void ERK4_MLTS_Lshape_MehlinExact_conv_test(int argc, char **argv);
...
void ERK4_MLTS_Lshape_MehlinExact_conv_test(int argc, char **argv)
{
    ...
    conv_fname << out_dir << "/lshape_mlts_convergence_mehlin_k_" << mesh_k << ".txt";
    ...
    for (int N : level_N) {
        ...
        // Pell_dofs[b] = P_b in the paper: ALL dofs of band b AND every
        // FINER band (b+1,...,L-1) -- built cumulatively from the finest
        // level down, since P_b = P_{b+1} union (this band's own dofs).
        // Split by cell/face (unshifted dof indices) so the overlap BFS
        // below can walk the Kfc sparsity graph directly, the same way
        // build_LTS_subblock's halo_rings does.
        ...
        // "Overlap" (Almquist-Mehlin Section 3.2, Table 2): widening P_ell
        // by a few grid points beyond the strict coarse/fine boundary is
        // reported REQUIRED for LTS-RK stability/accuracy -- 3 rings for
        // RK4. Expand each Pell_c[b]/Pell_f[b] (b < L-1) by `overlap_rings`
        // BFS hops through the Kfc sparsity graph, reaching a few extra
        // dofs on the COARSER side (bands < b) beyond the boundary.
        ...
        std::vector<std::vector<int>> Pell_dofs(L);
        for (int b = 0; b < L; ++b) {
            Pell_dofs[b] = Pell_c[b];
            for (int g : Pell_f[b]) Pell_dofs[b].push_back((int)erk_an.n_c_dof() + g);
            std::sort(Pell_dofs[b].begin(), Pell_dofs[b].end());
        }
        ...
        // Back to the STABLE structure (own_w_i recomputed fresh inside
        // the p_i loop, matching GlobalExact) -- the "freeze once per
        // visit, full-len window" variant blew up even at the trivial
        // L=2 case (root level, P_0=I, no masking involved at all), so
        // that failure is orthogonal to masking/overlap and cannot be
        // fixed by it: it is a plain RK-stability problem (a single
        // low-order polynomial cannot represent B's fastest eigenvalues,
        // inherited from direct coupling to the fine region, over a
        // window p_i times too large). Reverting to fresh-recompute
        // isolates the ONE remaining untested variable: overlap on
        // Pell_dofs (see the eq., no longer P_ell = "levels >= ell"
        // alone, but widened by `overlap_rings` extra BFS hops into the
        // coarser side, exactly as Almquist-Mehlin Section 3.2
        // prescribes -- 3 rings for RK4, Table 2). Without this, our
        // earlier P_ell-masked test used a bare, non-overlapping
        // partition, which the paper itself demonstrates is unstable/
        // inaccurate even at a SINGLE level (Fig. 1a).
        ...
        recurse = [&](int i, RealType t_start, RealType len, Matrix<RealType,Dynamic,1>& xn,
            ...
            for (int m = 0; m < p_i; ++m) {
                ...
                erk_an.erk_weight_LTS_coarse_mehlin(xn, Pell_dofs[i], active_dofs_band[i], own_w_i, Fn, Fn12, Fn1, dtau_i);
