...
void ERK4_MLTS_Lshape_DiagN2_test(int argc, char **argv);
...
void ERK4_MLTS_Lshape_DiagN2_test(int argc, char **argv)
{
    ...
    for (int N : level_N) {
        ...
        size_t nt = 1;
        if (const char* env_nt = std::getenv("MLTS_DIAG_NT")) nt = (size_t)std::atoi(env_nt);
        dt_macro = tf / std::max<size_t>(1, static_cast<size_t>(std::round(tf / dt_macro)));
        dt_level[0] = dt_macro;
        std::cout << bold << cyan << "      nt=" << nt << " (DIAG single-step) dt_macro=" << dt_macro << reset << std::endl;

        // GLOBAL band projectors, for the exact reference computed over
        // the SAME single macro-step, on the SAME real N=2 mesh.
        size_t n_dof_diag = x_dof.rows();
        ...
        for (int b = 0; b < L; ++b) {
            ...
            P_band[b] = SparseMatrix<RealType>(n_dof_diag, n_dof_diag);
        ...
        recurse = [&](int i, RealType t_start, RealType len, Matrix<RealType,Dynamic,1>& xn,
            ...
            for (int m = 0; m < p_i; ++m) {
                ...
                // UPDATE: the earlier ancestor_w attempt (see old note,
                // kept below for history) was worse because it STILL added
                // `combined = shifted + own_w_i` on top afterward, double-
                // counting `shifted`'s order-0 term (already folded into
                // own_w_i's MinvF0_tot by ancestor_w). Folding `shifted` in
                // via ancestor_w AND THEN using own_w_i directly (no extra
                // addition) for both the recursion and the closed-form
                // advance fixes the L=5/N=2 blow-up seen with plain
                // additive `combined` in a closed-form Simpson quadrature,
                // and brings the real-mesh residual down from ~7-9x to
                // ~5.9x (see ERK4_MLTS_Lshape_RestrictedExact_conv_test.hpp).
                erk_an.erk_weight_LTS_coarse_v2(xn, blocks[i], own_w_i, Fn, Fn12, Fn1, dtau_i, &shifted);
                if (i == 0 && m == 0) {
                    std::vector<Matrix<RealType,Dynamic,1>> own_w_i_exact(4);
                    for (int j = 0; j < 4; ++j) own_w_i_exact[j] = Matrix<RealType,Dynamic,1>::Zero(n_dof);
                    erk_an.erk_weight_LTS_coarse_global(xn, active_dofs_band[i], own_w_i_exact, Fn, Fn12, Fn1, dtau_i);
                    std::cout << bold << yellow << "      [DIAG] band0 own_w_i restricted vs exact, cell+face:" << reset << std::endl;
                    for (int j = 0; j < 4; ++j) {
                        RealType max_c = 0.0, max_f = 0.0;
                        for (int g : own_c[0]) max_c = std::max(max_c, std::abs(own_w_i[j](g) - own_w_i_exact[j](g)));
                        for (int g : own_f[0]) { int gg=(int)erk_an.n_c_dof()+g; max_f = std::max(max_f, std::abs(own_w_i[j](gg) - own_w_i_exact[j](gg))); }
                        RealType mag_c = 0.0, mag_f = 0.0;
                        for (int g : own_c[0]) mag_c = std::max(mag_c, std::abs(own_w_i_exact[j](g)));
                        for (int g : own_f[0]) { int gg=(int)erk_an.n_c_dof()+g; mag_f = std::max(mag_f, std::abs(own_w_i_exact[j](gg))); }
                        std::cout << bold << cyan << "         w[" << j << "]: max|diff|_c=" << max_c << " (mag " << mag_c
                                  << ")  max|diff|_f=" << max_f << " (mag " << mag_f << ")" << reset << std::endl;
                    }
                    // Also check band0's HALO faces specifically (the leak
                    // into an adjacent band's own interface face).
                    RealType max_halo_f = 0.0, mag_halo_f = 0.0;
                    for (size_t hi = blocks[0].n_own_f; hi < blocks[0].active_f.size(); ++hi) {
                        int local = blocks[0].active_f[hi];
                        int gg = (int)erk_an.n_c_dof() + local;
                        for (int j = 0; j < 4; ++j) {
                            max_halo_f = std::max(max_halo_f, std::abs(own_w_i[j](gg) - own_w_i_exact[j](gg)));
                            mag_halo_f = std::max(mag_halo_f, std::abs(own_w_i_exact[j](gg)));
                        }
                    }
                    std::cout << bold << cyan << "         halo faces (n=" << (blocks[0].active_f.size()-blocks[0].n_own_f)
                              << "): max|diff|=" << max_halo_f << " (mag " << mag_halo_f << ")" << reset << std::endl;
                }
                // own_w_i already carries the ancestor's cross-term-correct
                // contribution (folded in via ancestor_w above) -- no extra
                // additive `combined` needed (see note above).
        ...
        // GLOBAL exact recursion (own_w_i via erk_weight_LTS_coarse_global,
        // terminal via erk_weight_LTS_fine, both unrestricted), run on a
        // SEPARATE copy of the SAME initial state, for the SAME single
        // macro-step, on the SAME real N=2 mesh -- so any divergence from
        // the restricted result below is due ONLY to the restriction
        // itself, not to mesh/threshold/CFL differences.
        Matrix<RealType, Dynamic, 1> x_dof_global = x_dof;
        {
            std::function<void(int, RealType, RealType, Matrix<RealType,Dynamic,1>&, const std::vector<Matrix<RealType,Dynamic,1>>&)> recurse_g;
            recurse_g = [&](int i, RealType t_start, RealType len, Matrix<RealType,Dynamic,1>& xn,
                             const std::vector<Matrix<RealType,Dynamic,1>>& w_accum) {
                if (i == L - 1) {
                    Matrix<RealType, Dynamic, 1> Fm = eval_F(t_start), Fmh = eval_F(t_start + 0.5*len), Fm1 = eval_F(t_start + len);
                ...
                for (int m = 0; m < p_i; ++m) {
                    ...
                    recurse_g(i + 1, t_start + tm, dtau_i, xn, combined);
                }
            };
            for (size_t it = 1; it <= nt; it++) {
                RealType tn = ti + dt_macro * (it - 1);
                recurse_g(0, tn, dt_macro, x_dof_global, zero_w);
            }
        }
        ...
        // Per-band comparison: restricted (x_dof) vs global exact
        // (x_dof_global), same single macro-step, real N=2 mesh.
        std::cout << bold << yellow << "      [DIAG] per-band max|restricted - global| after 1 macro-step:" << reset << std::endl;
        for (int b = 0; b < L; ++b) {
            RealType max_c = 0.0, max_f = 0.0;
            int argmax_c = -1, argmax_f = -1;
            for (int g : own_c[b]) {
                RealType d = std::abs(x_dof(g) - x_dof_global(g));
                if (d > max_c) { max_c = d; argmax_c = g; }
            }
            for (int g : own_f[b]) {
                int gg = (int)erk_an.n_c_dof() + g;
                RealType d = std::abs(x_dof(gg) - x_dof_global(gg));
                if (d > max_f) { max_f = d; argmax_f = gg; }
            }
            std::cout << bold << cyan << "         band" << b << ": max|diff|_c=" << max_c
                      << " (dof " << argmax_c << ")  max|diff|_f=" << max_f
                      << " (dof " << argmax_f << ")" << reset << std::endl;
        }
