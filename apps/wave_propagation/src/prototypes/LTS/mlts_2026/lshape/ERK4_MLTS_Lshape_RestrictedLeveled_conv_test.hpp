...
#include <functional>
//
// EXPERIMENTAL: tests the "P_ell-faithful masking" hypothesis for the
// restricted multi-level driver. erk_weight_LTS_coarse_v2 (used by
// ERK4_MLTS_Lshape_RestrictedExact_conv_test.hpp) computes each band's
// own-dynamics B-chain from an UNMASKED halo (both finer AND coarser
// neighbours) -- unlike Almquist-Mehlin eq. (20)-(22), where B_L = B*P_L
// masks OUT every coarser (ancestor) level at EVERY application of B in
// the chain, not just once at the end. The unmasked version lets a
// band's chain re-read its parent's CURRENT value, double-counting a
// contribution already carried separately via the Taylor-shifted
// `ancestor_w`/`combined` term -- a plausible root cause of both the
// ~5.9x residual at L=5 (N=2) and the instability (NaN) at L=8 (N=3),
// and of why widening the halo (8->20 rings) made things WORSE (more
// parent contamination, not more legitimate reach).
//
// This driver uses build_LTS_subblock_leveled + erk_weight_LTS_coarse_v3
// (see erk_coupling_hho_scheme.hpp), which partition the halo into
// finer/coarser and mask the B-chain to "own+finer" only (P_ell) at
// every step, while keeping the one-time final output leak into the
// coarser halo (the correct (P_ell-P_{ell+1}) term, unaffected). If the
// hypothesis is right, a SMALL, FIXED, L-INDEPENDENT halo should now
// suffice at any depth -- tested here with halo_rings=4 (vs 8, and
// vs the 20 that made the old driver worse).
//
// Usage example: ../../../wave_propagation -k3 -s0 -r0 -c0 -f0 -e0

void ERK4_MLTS_Lshape_RestrictedLeveled_conv_test(int argc, char **argv);
...
void ERK4_MLTS_Lshape_RestrictedLeveled_conv_test(int argc, char **argv)
{
    ...
    conv_fname << out_dir << "/lshape_mlts_convergence_leveled_k_" << mesh_k << ".txt";
    ...
    for (int N : level_N) {
        ...
        // Per-DOF band index, needed by build_LTS_subblock_leveled to
        // classify each halo entry as "finer" (kept in the B-chain) or
        // "coarser" (excluded from the B-chain, P_ell-style).
        std::vector<int> dof_band_c(erk_an.n_c_dof());
        for (int b = 0; b < L; ++b) for (int g : own_c[b]) dof_band_c[g] = b;
        std::vector<int> dof_band_f(erk_an.n_f_dof());
        for (int b = 0; b < L; ++b) for (int g : own_f[b]) dof_band_f[g] = b;

        // RESTRICTED per-band sub-blocks (own+halo), "leveled": the halo
        // is partitioned into finer/coarser and the coarse-role B-chain
        // (erk_weight_LTS_coarse_v3) masks its input to own+finer at
        // EVERY step, matching Almquist-Mehlin's B_L=BP_L (eq. 20-22)
        // instead of leaking the parent's current value into the chain.
        // Halo kept small and FIXED (not scaled with L) to test whether
        // removing that leak also removes the need for a growing halo.
        const int halo_rings = 8;
        // Overlap (Almquist-Mehlin Section 3.2, Table 2: 3 rings for
        // RK4): confirmed on the GLOBAL-matrix Mehlin reproduction
        // (ERK4_MLTS_Lshape_MehlinExact_conv_test.hpp) to close a 2.93x
        // mismatch down to ~0.4% at L=5. Testing here whether it also
        // fixes the restricted/halo-based leveled driver's own
        // mismatch (x8.0-8.6 without overlap).
        const int overlap_rings = std::getenv("MLTS_OVERLAP_RINGS") ? std::atoi(std::getenv("MLTS_OVERLAP_RINGS")) : 3;
        std::vector<erk_coupling_hho_scheme<RealType>::LTS_subblock_set_leveled> blocks(L);
        for (int b = 0; b < L; ++b) {
            blocks[b] = erk_an.build_LTS_subblock_leveled(own_c[b], own_f[b], halo_rings,
                                                            dof_band_c, dof_band_f, b, overlap_rings);
            std::cout << bold << cyan << "      band" << b << ": own_c=" << own_c[b].size()
                      << " own_f=" << own_f[b].size()
                      << "  (+finer_c=" << blocks[b].n_finer_c << " +coarser_c="
                      << (blocks[b].active_c.size() - blocks[b].n_own_c - blocks[b].n_finer_c) << ")"
                      << reset << std::endl;
        }
        ...
        recurse = [&](int i, RealType t_start, RealType len, Matrix<RealType,Dynamic,1>& xn,
            ...
            for (int m = 0; m < p_i; ++m) {
                ...
                erk_an.erk_weight_LTS_coarse_v3(xn, blocks[i], own_w_i, Fn, Fn12, Fn1, dtau_i, &shifted);
