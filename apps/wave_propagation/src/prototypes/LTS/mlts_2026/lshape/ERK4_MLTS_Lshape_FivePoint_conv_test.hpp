...
void ERK4_MLTS_Lshape_FivePoint_conv_test(int argc, char **argv);
...
void ERK4_MLTS_Lshape_FivePoint_conv_test(int argc, char **argv)
{
    ...
    // 5 points, LOG-EQUIDISTANT in h_max (geometric spacing), spanning
    // N=0..N=2's h-range only (h_max in [0.088, 0.354]) -- NOT out to
    // N=3/N=4, whose p_global explodes exponentially (32^N) as an
    // intrinsic consequence of the graded-mesh theory (matching the
    // corner singularity's regularity for k=3), not an artifact of mesh
    // choice; reaching a log-equidistant point near N=3's h would cost
    // about as much as N=3 itself, defeating the purpose.
    //   N=0  (n0=4, ell=0): h_max=0.353553  p_global=1
    //   0b   (n0=6, ell=0): h_max=0.235702  p_global~16
    //   N=1  (n0=4, ell=1): h_max=0.176777  p_global=32
    //   1b   (n0=6, ell=1): h_max=0.117851  p_global~512
    //   N=2  (n0=4, ell=2): h_max=0.088388  p_global=1024
    // Successive ratios: 1.50 / 1.333 / 1.50 / 1.333 (geometric mean per
    // step = sqrt(2) exactly, matching the total x4 span over 4 steps --
    // the closest achievable to perfect log-equidistance given n0 must be
    // an even integer). The "0b"/"1b" meshes use a theory-corrected
    // corner depth, total_depth = round(1.5*(k+1)*(ell + log2(n0/4))),
    // anchored to the validated n0=4 reference so the SAME order-(k+1)
    // grading applies at every point (see mesh_generation/lshape_graded/
    // generate_lshape_graded_mesh.py's docstring). All points use the
    // GLOBAL EXACT algorithm (erk_weight_LTS_fine / erk_weight_LTS_coarse_global,
    // no multi-level restriction), proven to machine precision against
    // the trusted 2-level reference -- guarantees the observed order is
    // the TRUE discretization order, not contaminated by the restricted
    // driver's residual.
    std::vector<std::string> level_labels = {"0", "0b", "1", "1b", "2"};
    if (const char* env_n = std::getenv("MLTS_LEVELS")) {
        level_labels.clear();
        std::istringstream iss(env_n);
        std::string tok;
        while (iss >> tok) level_labels.push_back(tok);
    }

    const std::string out_dir = "lshape/results";
    std::filesystem::create_directories(out_dir);
    std::ostringstream conv_fname;
    conv_fname << out_dir << "/lshape_mlts_convergence_5pt_k_" << mesh_k << ".txt";
    std::ofstream conv_log(conv_fname.str());
    conv_log << "# label  h_max  h_min  p_global  L_levels  L2_error_pressure  wall_time_s\n";

    for (const std::string& N : level_labels) {
        ...
        // Grote-Diaz-faithful, GLOBAL-matrix recursion (validated to
        // machine precision at L=2..5, no dependence on L): every level
        // i < L-1 recomputes its OWN contribution fresh, from the CURRENT
        // global state, at every short sub-step, and ACCUMULATES it
        // (sums, exactly like their v := w + Sum w_k) with what it
        // inherited from its ancestors before Taylor-shifting and
        // recursing. Only the terminal level gets genuine RK4 dynamics;
        // every other level's own dofs are updated purely through the
        // unrestricted leak of the global Kcc/Kcf/Kfc operators inside
        // erk_weight_LTS_fine's Taylor_w term, chained across the
        // recursion's many nested visits.
