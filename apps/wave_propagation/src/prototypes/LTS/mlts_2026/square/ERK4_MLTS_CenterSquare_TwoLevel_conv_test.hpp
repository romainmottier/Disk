...
void ERK4_MLTS_CenterSquare_TwoLevel_conv_test(int argc, char **argv);
...
void ERK4_MLTS_CenterSquare_TwoLevel_conv_test(int argc, char **argv)
{
    ...
    // Two-level (L=2) p-sweep: `pfam` selects a FIXED-p mesh family
    // (p_global = 2^pfam, constant across the whole N sweep -- see
    // mesh_generation/center_square_fixed's centersq_pfix{pfam}_k3_N{N}.txt),
    // and the coarse/fine threshold below is hardcoded to a single split
    // (background vs. everything locally refined, regardless of how many
    // internal quadtree sub-sizes the refined region itself contains).
    //
    // NOTE: this still runs through the SAME general multilevel recursive
    // machinery as the rest of this investigation (erk_weight_LTS_coarse_
    // Pell_sparse, Pell_c/Pell_f projectors, overlap rings, the generic
    // `recurse` below) -- it is NOT a separate, purpose-built two-level-only
    // implementation like ERK4_LTS_conv_test.hpp. What makes this "two-level"
    // is only that `thresholds` holds a single split, forcing L=2 (one
    // coarse band + one terminal/fine band) regardless of `pfam`. Do not
    // confuse `p` (the mesh ratio h_max/h_min, i.e. how many RK4 substeps
    // happen inside the fine band per coarse step) with `L` (the number of
    // levels/bands) -- L is held fixed at 2 for every curve in this sweep;
    // only p varies.
    ...
    conv_fname << out_dir << "/centersquare_twolevel_p" << (1 << pfam) << "_mlts_convergence_k_" << mesh_k << ".txt";
    ...
    for (int N : level_N) {
        ...
        std::cout << bold << red << "\n   ================ MESH LEVEL N=" << N << " (p=2^" << pfam << ") ================" << reset << std::endl;
        ...
        {
            thresholds.push_back(h_max * 0.999);
        }
