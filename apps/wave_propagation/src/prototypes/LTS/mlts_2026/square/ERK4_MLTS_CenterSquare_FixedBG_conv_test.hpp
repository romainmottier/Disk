...
// ERK4_MLTS_CenterSquareCoarse_PellSparse_conv_test.hpp, pointed at a
// DIFFERENT mesh family (centersquareadaptive_graded, generated by
// mesh_generation/center_square_fixed/generate_adaptive_family.py:
// n0=4, halo=0, extra=2*N, so p_global = 4^N GROWS with N -- 1, 4, 16,
// 64, 256 across N=0..4).
//
...
// derived L follows 1, 1, 2, 3, 4, 5 across N=0..5 -- a genuine,
// monotonically-growing multilevel trace (unlike the older
// centersquarecoarse_graded family, extra=N, whose p only reached 16
// and L only reached 2 at its very last point).
//
// L=1 (N=0,1 in this family) is NEVER used anywhere in this
...
// the default sweep below SKIPS it outright: default level_N = {2, 3,
// 4, 5} (L=2..5), not {0..4}.
//
...
void ERK4_MLTS_CenterSquare_Adaptive_conv_test(int argc, char **argv)
{
    ...
    // L=1 (N=0,1) is never used in this investigation -- default sweep
    // starts at N=2 (L=2) outright, not N=0. MLTS_N_MIN/MLTS_N_MAX can
    // still narrow or (deliberately) widen this range.
    int n_min_default = 2, n_max_default = 5;
    ...
    conv_fname << out_dir << "/centersquareadaptive_pellsparse_mlts_convergence_k_" << mesh_k;
    ...
    for (int N : level_N) {
        ...
        mesh_fname << mesh_dir << "centersquareadaptive_graded_k" << mesh_k << "_N" << N << ".txt";
