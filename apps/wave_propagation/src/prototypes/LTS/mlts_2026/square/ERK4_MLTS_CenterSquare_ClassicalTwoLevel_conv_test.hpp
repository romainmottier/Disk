...
#include <functional>
//
// GENUINE classical two-level LTS-RK4 -- NOT the general multilevel Pell-
// sparse recursion used by ERK4_MLTS_CenterSquare_TwoLevel_conv_test.hpp
// (which forces L=2 out of the recursive band machinery shared with the
// deep-L drivers). This driver instead uses
// elastoacoustic_four_fields_assembler::assemble_P(msh, h_c, nb_layer) to
// build the diagonal Pcoarse/Pfine projectors (threshold cell diameter vs
// h_c, with a vertex-sharing "layer" dilation around the fine region), and
// the plain erk_weight_LTS_coarse / erk_weight_LTS_fine functions -- ONE
// coarse call per macro step followed by `p` fine substeps, no band-index
// bookkeeping, no recursion. This is exactly the methodology of
// ERK4_LTS_conv_test.hpp / ERK4_LTS_Lshape_conv_test.hpp (the latter
// confirms this machinery works unmodified on a pure-acoustic domain, empty
// e_material, nb_layer=3 recommended for a first-order mixed system as p
// grows).
//
// Same square/sinusoidal problem, same mesh family (centersq_pfix{pfam}),
// same tf/cfl_factor as ERK4_MLTS_CenterSquare_TwoLevel_conv_test.hpp, so
// the two drivers are directly comparable -- one "classical", one "MLTS
// machinery at L=2".
//
// Usage example: ../../../wave_propagation -k3 -s0 -r0 -c0 -f0 -e0

void ERK4_MLTS_CenterSquare_ClassicalTwoLevel_conv_test(int argc, char **argv);
...
void ERK4_MLTS_CenterSquare_ClassicalTwoLevel_conv_test(int argc, char **argv)
{
    ...
    // Same FIXED-p mesh families as ERK4_MLTS_CenterSquare_TwoLevel_conv_test
    // (p_global = 2^pfam, constant across the whole N sweep).
    const int pfam = std::getenv("MLTS_PFAM") ? std::atoi(std::getenv("MLTS_PFAM")) : 1;
    ...
    // grows; we sweep p up to 32 here, same order of magnitude, so default
    // to 3 for safety (overridable).
    ...
    conv_fname << out_dir << "/centersquare_classicaltwolevel_p" << (1 << pfam) << "_convergence_k_" << mesh_k << ".txt";
    ...
    for (int N : level_N) {
        ...
        std::cout << bold << red << "\n   ================ MESH LEVEL N=" << N << " (classical p=2^" << pfam << ") ================" << reset << std::endl;
