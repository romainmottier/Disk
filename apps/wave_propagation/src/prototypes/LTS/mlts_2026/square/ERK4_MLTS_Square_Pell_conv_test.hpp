...
void ERK4_MLTS_Square_Pell_conv_test(int argc, char **argv);
...
void ERK4_MLTS_Square_Pell_conv_test(int argc, char **argv)
{
    ...
    conv_fname << out_dir << "/square_pell_mlts_convergence_k_" << mesh_k << ".txt";
    ...
    for (int N : level_N) {
        ...
        // LITERAL Almquist-Mehlin P_ell + overlap construction (eq. 20-22,
        // Section 3.2/Table 2), applied to EVERY band (not just the
        // terminal): each band's submatrix is built from its own dofs +
        // every FINER band's dofs (P_ell, "level ell and every finer
        // level") + a few overlap rings into the coarser side, matching
        // ERK4_MLTS_Lshape_MehlinRestricted_conv_test.hpp exactly. Kept
        // as its own driver (as opposed to the HYBRID one, which uses
        // GLOBAL matrices for every non-terminal band) specifically to
        // test the paper's own overlap mechanism in isolation, with
        // nothing else substituted in.
