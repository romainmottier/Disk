...
void ERK4_LTS_stab_acou(int argc, char **argv)
{
    std::cout << std::endl << bold << red << "   ERK4-LTS STABILITY ANALYSIS ACOUSTIC" << std::endl << std::endl;
    ...
    timecounter tc, cpu;
    cpu.tic();
    const bool save_eigenvalues_Q = true;

    // Debug diagnostic: compare C_lts (erk_weight_LTS_fine) vs C_std (Butcher
    // + erk_weight) at a few dt values, to check that both paths agree when
    // p=1 (no local refinement: Pfine=I, coarse weights=0). See the debug
    // block after the stability sweep below.
    const bool debug_compare_lts_std_Q = false;
    ...
    for (int s = 0; s < n_pts; ++s) {
        ...
        if (es.info() == Eigen::Success) {
            ...
            if (save_eigenvalues_Q && rho > 1.0 - 0.05) {
                ...
                ev_file << "# dt=" << dt_s << "  rho=" << std::setprecision(15) << rho << "\n";
                ev_file << "# real  imag\n";
    ...
    // =========================================================================
    // DEBUG : compare C_lts vs C_std (optional, off by default)
    // =========================================================================
