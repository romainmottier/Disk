...
// cell_is_fine: vector indexed by global cell_id (storage->surfaces order),
    // built by assemble_P after layer propagation. Reflects the exact fine/coarse
    // partition used by the LTS scheme, including protection layers.
    // Pass an empty vector to fall back to the simple diameter < h_c criterion.
    static void write_silo_four_fields_elastoacoustic_LTS(
        ...
        bool cell_centered_Q, double h_c,
        const std::vector<bool> & cell_is_fine = {}) {
        ...
        if (cell_centered_Q)
        {
            ...
            for (auto& e_chunk : e_material)
            {
                ...
                auto cell_basis = make_vector_monomial_basis(msh, cell, hho_di.cell_degree());
                Matrix<RealType, Dynamic, 1> vec_x_cell_dof =
                    x_dof.block(e_cell_ind * e_cell_dof + n_ten_cbs, 0, n_vec_cbs, 1);
                auto t_phi = cell_basis.eval_functions(bar);
                assert(t_phi.rows() == cell_basis.size());
                auto uh = disk::eval(vec_x_cell_dof, t_phi);
                approx_ux.at(e_chunk.first)     = uh(0, 0);
            ...
            for (auto& a_chunk : a_material)
            {
                ...
                auto cell_basis = make_scalar_monomial_basis(msh, cell, hho_di.cell_degree());
                Matrix<RealType, Dynamic, 1> scalar_cell_dof =
                    x_dof.block(a_cell_ind * a_cell_dof + n_vel_scal_cbs + n_elastic_cell_dof,
                                0, n_scal_cbs, 1);
                auto t_phi  = cell_basis.eval_functions(bar);
                RealType uh = scalar_cell_dof.dot(t_phi);
                approx_u.at(a_chunk.first) = uh;
        ...
        {
            // Filling with nan (useful in Paraview to distinguish elastic/acoustic)
            approx_ux.resize(num_points,     0.0 / 0.0);
            approx_uy.resize(num_points,     0.0 / 0.0);
            approx_uy_bis.resize(num_points, 0.0 / 0.0);
            approx_u.resize(num_points,      0.0 / 0.0);
            ...
            for (auto& e_chunk : e_material)
            {
                ...
                for (size_t l = 0; l < points.size(); l++)
                {
                    ...
                    approx_ux.at(pt_id)     = uh(0, 0);
                    approx_uy.at(pt_id)     = uh(1, 0);
                    approx_uy_bis.at(pt_id) = std::sqrt(uh(0,0)*uh(0,0) + uh(1,0)*uh(1,0));
                }
            }
            ...
            for (auto& a_chunk : a_material)
            {
                ...
                for (size_t l = 0; l < points.size(); l++)
                {
                    ...
                    auto pt_coord = *std::next(msh.points_begin(), pt_id);
                    auto t_phi    = cell_basis.eval_functions(pt_coord);
                    approx_u.at(pt_id) = scalar_cell_dof.dot(t_phi);
                }
            }
        }

        // Build the fine/coarse field for Silo visualisation.
        //
        // The silo loop iterates in msh order (silo_id = 0, 1, ..., num_cells-1).
        // cell_is_fine is indexed by storage->surfaces global id = msh.lookup(cell).
        // The two orderings can differ, so we use msh.lookup(cell) explicitly.
        //
        // If cell_is_fine was provided by assemble_P (includes protection layers),
        // use it. Otherwise fall back to the simple diameter < h_c test.
        std::vector<double> fine_coarse(num_cells, 0.0);
        {
            size_t silo_id = 0;
            for (auto& cell : msh)
            {
                const size_t global_id = msh.lookup(cell);
                if (!cell_is_fine.empty())
                    fine_coarse[silo_id] = cell_is_fine[global_id] ? 1.0 : 0.0;
                else
                    fine_coarse[silo_id] = (diameter(msh, cell) < h_c) ? 1.0 : 0.0;
                silo_id++;
            }
        }
        ...
        if (cell_centered_Q)
        {
            ...
            disk::silo_zonal_variable<double> vhy_bis_silo("norm_v",approx_uy_bis);
            disk::silo_zonal_variable<double> vh_silo("vh",         approx_u);
            disk::silo_zonal_variable<double> fc_silo("fine_cells", fine_coarse);
            silo.add_variable("mesh", vhx_silo);
            silo.add_variable("mesh", vhy_silo);
            silo.add_variable("mesh", vhy_bis_silo);
            silo.add_variable("mesh", vh_silo);
            silo.add_variable("mesh", fc_silo);
        }
        else
        {
            disk::silo_nodal_variable<double> vhx_silo("vhx",       approx_ux);
    ...
    // ================================================================================
    // Generate Python convergence plot script for ERK4-LTS conv tests.
    // Un seul graphe avec deux courbes par k :
    //   - L2 HHO  (trait plein)   = e_L2 + a_L2
    //   - L2 dG   (pointille)     = e_dG + a_dG
    // Triangle de reference uniquement pour k=k_max.
    //
    // Log files expected:  lts_l_{l}_n_{n}_k_{k}_s_4_p_{p}_discret_{d}.txt
    // ================================================================================
    static void generate_LTS_convergence_script(
            size_t k_max,
            size_t l_max,
            size_t n_divs,
            size_t substeps,
            bool   hdg_stabilization,
            const std::string & script_name = "plot_LTS_convergence.py") {

        std::ofstream py(script_name);

        // ── Header ────────────────────────────────────────────────────────────
        py << "#!/usr/bin/env python3\n";
        py << "# Auto-generated by postprocessor::generate_LTS_convergence_script\n\n";
        py << "import os\n";
        py << "import math\n";
        py << "import numpy as np\n";
        py << "import matplotlib\n";
        py << "import matplotlib.pyplot as plt\n";
        py << "matplotlib.rcParams['text.usetex'] = True\n";
        py << "from matplotlib.lines import Line2D\n\n";

        py << "current_file_path = os.path.abspath(__file__)\n";
        py << "current_directory = os.path.dirname(current_file_path)\n";
        py << "os.chdir(current_directory)\n\n";

        // ── Parameters ───────────────────────────────────────────────────────
        py << "k_max    = " << k_max                        << "\n";
        py << "l_max    = " << l_max                        << "\n";
        py << "n_divs   = " << n_divs                       << "\n";
        py << "substeps = " << substeps                     << "\n";
        py << "discret  = " << (hdg_stabilization ? 1 : 0) << "\n\n";

        // ── Styles per k ─────────────────────────────────────────────────────
        py << "styles = {\n";
        py << "    1: {'color': 'darkgreen', 'marker': '^'},\n";
        py << "    2: {'color': 'orange',    'marker': 's'},\n";
        py << "    3: {'color': 'darkred',   'marker': 'o'},\n";
        py << "    4: {'color': 'navy',      'marker': 'd'},\n";
        py << "    5: {'color': 'purple',    'marker': 'v'},\n";
        py << "}\n\n";

        // ── Log parser ────────────────────────────────────────────────────────
        py << "def parse_log(filename):\n";
        py << "    if not os.path.isfile(filename):\n";
        py << "        print(f'Fichier introuvable : {filename}')\n";
        py << "        return None\n";
        py << "    data = {}\n";
        py << "    with open(filename) as f:\n";
        py << "        lines = f.readlines()\n";
        py << "    section = None\n";
        py << "    for s in (l.strip() for l in lines):\n";
        py << "        if 'Space step =' in s:\n";
        py << "            data['h'] = float(s.split('=')[1])\n";
        py << "        elif s == 'Elastic region :':\n";
        py << "            section = 'elastic'\n";
        py << "        elif s == 'Acoustic region :':\n";
        py << "            section = 'acoustic'\n";
        py << "        elif 'L2 errors of dG' in s:\n";
        py << "            section = 'dg'\n";
        py << "        elif s.startswith('L2-norm error') and section in ('elastic', 'acoustic'):\n";
        py << "            val = float(s.split('=')[1])\n";
        py << "            data['e_L2' if section == 'elastic' else 'a_L2'] = val\n";
        py << "        elif s.startswith('Acoustic region :') and section == 'dg':\n";
        py << "            data['a_dG'] = float(s.split(':')[1])\n";
        py << "        elif s.startswith('Elastic  region') and section == 'dg':\n";
        py << "            data['e_dG'] = float(s.split(':')[1])\n";
        py << "    return data if 'h' in data and 'e_L2' in data else None\n\n";

        // ── Collect data ──────────────────────────────────────────────────────
        py << "results = {k: {'h':[], 'e_L2':[], 'a_L2':[], 'e_dG':[], 'a_dG':[]}\n";
        py << "           for k in range(1, k_max+1)}\n\n";
        py << "for k in range(1, k_max+1):\n";
        py << "    for l in range(0, l_max+1):\n";
        py << "        fname = f'lts_l_{l}_n_{n_divs}_k_{k}_s_4_p_{substeps}_discret_{discret}.txt'\n";
        py << "        d = parse_log(fname)\n";
        py << "        if d is None: continue\n";
        py << "        for key in ('h', 'e_L2', 'a_L2', 'e_dG', 'a_dG'):\n";
        py << "            results[k][key].append(d.get(key, float('nan')))\n\n";

        // ── Sort helper ───────────────────────────────────────────────────────
        py << "def sorted_by_h(res_k, key):\n";
        py << "    pairs = sorted(zip(res_k['h'], res_k[key]))\n";
        py << "    return np.array([p[0] for p in pairs]), np.array([p[1] for p in pairs])\n\n";

        // ── Plot ──────────────────────────────────────────────────────────────
        py << "plt.figure(figsize=(8, 8))\n\n";
        py << "for k in range(1, k_max+1):\n";
        py << "    if not results[k]['h']: continue\n";
        py << "    col = styles[k]['color']\n";
        py << "    mrk = styles[k]['marker']\n\n";
        py << "    h,  e_L2 = sorted_by_h(results[k], 'e_L2')\n";
        py << "    _,  a_L2 = sorted_by_h(results[k], 'a_L2')\n";
        py << "    _,  e_dG = sorted_by_h(results[k], 'e_dG')\n";
        py << "    _,  a_dG = sorted_by_h(results[k], 'a_dG')\n";
        py << "    L2_hho = e_L2 + a_L2\n";
        py << "    L2_dg  = e_dG + a_dG\n\n";
        py << "    # HHO -- trait plein\n";
        py << "    plt.loglog(h, L2_hho, marker=mrk, linestyle='-',  color=col)\n";
        py << "    # dG  -- pointille\n";
        py << "    plt.loglog(h, L2_dg,  marker=mrk, linestyle='--', color=col, alpha=0.7)\n\n";

        // ── Legend ────────────────────────────────────────────────────────────
        py << "handles = [\n";
        py << "    Line2D([], [], color='k', linestyle='-',\n";
        py << "           label=r'$\\mathbf{||\\cdot||_{\\mathrm{HHO}}}$'),\n";
        py << "    Line2D([], [], color='k', linestyle='--',\n";
        py << "           label=r'$\\mathbf{||\\cdot||_{\\mathrm{dG}}}$'),\n";
        py << "]\n";
        py << "for k in range(1, k_max+1):\n";
        py << "    if not results[k]['h']: continue\n";
        py << "    h,  e_L2 = sorted_by_h(results[k], 'e_L2')\n";
        py << "    _,  a_L2 = sorted_by_h(results[k], 'a_L2')\n";
        py << "    L2  = e_L2 + a_L2\n";
        py << "    rate  = np.polyfit(np.log(h), np.log(L2), 1)[0]\n";
        py << "    order = int(round(rate))\n";
        py << "    lbl   = rf'$\\mathbf{{k={k}}} \\approx \\mathcal{{O}}(h^{{{order}}})$'\n";
        py << "    handles.append(Line2D([], [], color=styles[k]['color'],\n";
        py << "                          marker=styles[k]['marker'],\n";
        py << "                          linestyle='-', label=lbl))\n";
        py << "plt.legend(handles=handles, labelcolor='linecolor',\n";
        py << "           loc='lower right', framealpha=1, fontsize='12.5')\n\n";

        // ── Axis formatting ───────────────────────────────────────────────────
        py << "plt.yscale('log')\n";
        py << "plt.xscale('log')\n";
        py << "plt.minorticks_off()\n";
        py << "plt.xlabel(r'$h$', fontsize=25, labelpad=20)\n";
        py << "plt.ylabel(r'$\\textsc{L}^2\\textsc{-error}$', fontsize=25, labelpad=20)\n";
        py << "plt.tick_params(axis='x', which='major', pad=10)\n";
        py << "plt.tick_params(axis='y', which='major', pad=10)\n";
        py << "plt.xticks(fontsize=20)\n";
        py << "plt.yticks(fontsize=20)\n\n";

        // ── Reference triangle — uniquement pour k=k_max ─────────────────────
        py << "ax = plt.gca()\n";
        py << "ax.figure.canvas.draw()\n";
        py << "xlim = ax.get_xlim()\n";
        py << "ylim = ax.get_ylim()\n";
        py << "lx0 = math.log10(xlim[0]); lx1 = math.log10(xlim[1])\n";
        py << "ly0 = math.log10(ylim[0]); ly1 = math.log10(ylim[1])\n";
        py << "triangle_x2 = 10**(lx0 + 0.55*(lx1-lx0))\n";
        py << "triangle_x  = 10**(lx0 + 0.90*(lx1-lx0))\n";
        py << "triangle_y  = 10**(ly0 + 0.04*(ly1-ly0))\n";
        py << "p   = k_max\n";
        py << "col = styles[p]['color']\n";
        py << "y_top = math.exp(p * math.log(triangle_x / triangle_x2) + math.log(triangle_y))\n";
        py << "tri   = np.array([[triangle_x,  triangle_y],\n";
        py << "                   [triangle_x,  y_top],\n";
        py << "                   [triangle_x2, triangle_y]])\n";
        py << "plt.plot(tri[:,0], tri[:,1], color=col, linewidth=2.5)\n";
        py << "plt.plot([triangle_x2, triangle_x, triangle_x],\n";
        py << "         [triangle_y,  triangle_y, tri[1,1]],\n";
        py << "         color='black', linewidth=2.5)\n";
        py << "plt.text(triangle_x*1.04, y_top,\n";
        py << "         f'$\\\\mathbf{{{p}}}$', fontsize=15, color=col)\n\n";

        // ── Save ──────────────────────────────────────────────────────────────
        py << "plt.tight_layout()\n";
        py << "plt.savefig('conv_LTS.png', dpi=300, bbox_inches='tight')\n";
        py << "plt.savefig('conv_LTS.pdf', bbox_inches='tight')\n";
        py << "print('Saved conv_LTS.png / conv_LTS.pdf')\n";
        py << "plt.show()\n\n";

        // ── Summary table ─────────────────────────────────────────────────────
        py << "print('\\n=== Convergence rate summary (L2 HHO) ===')\n";
        py << "print(f'{\"k\":>4} {\"rate\":>8}')\n";
        py << "print('-'*14)\n";
        py << "for k in range(1, k_max+1):\n";
        py << "    if not results[k]['h']: continue\n";
        py << "    h,  e_L2 = sorted_by_h(results[k], 'e_L2')\n";
        ...
        py << "    _,  a_L2 = sorted_by_h(results[k], 'a_L2')\n";
        py << "    L2    = e_L2 + a_L2\n";
        py << "    rates = np.log(L2[1:]/L2[:-1]) / np.log(h[1:]/h[:-1])\n";
        py << "    for i, r in enumerate(rates):\n";
        py << "        print(f'k={k}: h={h[i]:.4f} -> h={h[i+1]:.4f} : rate={r:.3f}')\n";

        py.close();
        std::cout << bold << cyan << "      Python convergence script written : "
                  << script_name << reset << std::endl;
    }
