...
# # ---------------------------
# NumberofElements = 1000   # nombre d'éléments désiré
# MaxIterations = 500     # nombre max d'itérations pour l'algorithme
...
def generate_mesh(polygon_points, NumberofElements, MaxIterations, verbose=True):
    ...
    area = domain.CalculateArea()
    metrics = mesh_assessment(Node, Element, area, verbose=verbose)

    if verbose:
        print("Nodes shape:", Node.shape)
        print("Number of elements:", len(Element))

    return Node, Element, metrics
...
Node, Element, metrics = generate_mesh(polygon_points, NumberofElements=1000, MaxIterations=500)
